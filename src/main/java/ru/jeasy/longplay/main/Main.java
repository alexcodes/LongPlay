...
public class Main
{
    ...
    private static Method findLaunchMethod(File jfxRtPath, String fxClassPath) {
        Class[] argTypes = { Class.class, Class.class, String[].class};
        try
        {
            ...
            if (cp != null)
            {
                ...
                while (cp.length() > 0) {
                    ...
                    if (pathSepIdx < 0) {
                        File f = baseDir == null ? new File(cp) : new File(baseDir, cp);
    ...
    private static String lookupRegistry()
    {
        ...
        try
        {
            ...
            String[] fxVersions = (String[])mGetSubKeys.invoke(null, HKEY_LOCAL_MACHINE2, "Software\\Oracle\\JavaFX\\", 255);
            ...
            for (String fxVersion : fxVersions) {
                if ((fxVersion.startsWith("2.")) && (fxVersion.compareTo("2.1.0") >= 0)) {
                    ...
                    System.err.println("  Skip version " + fxVersion + " (required=" + "2.1.0" + ")");
                }
            ...
            String path = (String)mGet.invoke(null, HKEY_LOCAL_MACHINE, "Software\\Oracle\\JavaFX\\" + version, "Path");
    ...
    private static void processUpdateHook(String updateHookName)
    {
        ...
        try
        {
            ...
            Method mainMethod = hookClass.getMethod("main", new Class[] {String[].class});
    ...
    private static void showFallback() {
        SwingUtilities.invokeLater(new Runnable() { private final boolean val$jreError = false;
            ...
            public void run() { JFrame f = new JFrame("JavaFX Launcher");
                ...
                if (japp == null)
                {
                    japp = new NoJavaFXFallback(this.val$jreError, !this.val$jreError, "2.1.0");
    ...
    private static void errorExit()
    {
        try
        {
            Runnable runnable = new Runnable() {
                private final String val$string = null;
                ...
                public void run() { try { Class componentClass = Class.forName("java.awt.Component");
                    ...
                    showMessageDialogMethod.invoke(null, null, this.val$string, "JavaFX Launcher Error", ERROR_MESSAGE);
                }
                catch (Exception ex)
                {
                    ex.printStackTrace();
